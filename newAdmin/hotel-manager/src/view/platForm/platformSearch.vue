<template>
  <div class="singleTableSelf">
    <div class="selectHotelname"><span>当前选择的酒店：</span>{{selectHotelname}}</div>
    <!-- 表格操作 -->
        ...
        <a-spin :spinning="spinning">
            <div  v-for="(item,index) in figureList"  :key="index" :class="item.active?'active figureli':'figureli'" @click="customRowFu(item,index)" @mouseover="customOver(item,index)" @mouseout="customOut(item,index)" ref="wraps">
              <div class="figureli-top" :style="{'height':figureliHeight}">
                ...
                <div class="figureli-top-mask" v-if="item.mask"></div>
              </div>
...
export default {
  ...
  data () {
    return {
      ...
      table: {
        ...
        hotelSreach: { // 酒店选择
          btnClick: this.hotelSreachClick
        },
        custSreach: { // 自定义查询功能
  ...
  methods: {
    ...
    complete (selectCompanyObject, selectHotelObject) {
      this.selectHotelname = selectHotelObject[0].name
  ...
  mounted () {
    // console.log((this.$refs.wraps.clientWidth - 40) / 5)
  }
}
</script>
...
.active{border:1px solid #0ACAC7 !important}
.figureli{
    width: calc(20% - 8px);
    float: left;
