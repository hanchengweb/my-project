    ...
    <div class="clearfix">
      <div class="selectline clearfix">
          <div class="selectlineDiv">
              <span class="selectlineDivSpa">归属类型：</span>
              <a-select  style="width: 118px" @change="(value)=>changeOwntype(value)" :defaultValue="searchData.ownerTypeNam">
                  <a-select-option v-for="(item,index) in ownerTypeList" :value="item.code" :key="index">{{item.name}}</a-select-option>
              </a-select>
          </div>
          <div class="selectlineDiv">
              <button :class="[ownerTypeValue==1?'disabledClor':'' ,'ant-btn ant-colors selectBut']" :disabled="ownerTypeValue==1" v-if="ownType.length<=0" @click="selectObjs('ownType')">+添加归属标识匹配(可选)</button>
      ...
      <div class="selectline clearfix" v-if="selectShow">
          <div class="selectlineDiv"  >
              <span class="selectlineDivSpa">投放范围：</span>
              <a-select  style="width: 118px" @change="(value)=>changeScope(value)" :defaultValue="searchData.scopeNam">
                  <a-select-option v-for="(item,index) in scopeList" :value="item.code" :key="index">{{item.name}}</a-select-option>
              </a-select>
          </div>
          <div class="selectlineDiv">
              <button :class="[scopeValue==1?'disabledClor':'' ,'ant-btn ant-colors selectBut']" :disabled="scopeValue==1" v-if="targetObject.length<=0" @click="selectObjs('targetObject')">+添加投放对象匹配(必选)</button>
              <div class="selectResult ant-colors" v-else>
...
export default {
  ...
  methods: {
    ...
    complete (companyCode, hotelIds) {
      if (this.modalData.type == 'ownType') {
        ...
        if (hotelIds.length > 0) {
          ...
          this.searchComplete && this.searchComplete.completeOwntype && this.searchComplete.completeOwntype(this.ownerTypeValue, hotelIds[0])
        } else {
          this.selectOwnTypeObject = companyCode[0].name
          this.searchComplete && this.searchComplete.completeOwntype && this.searchComplete.completeOwntype(this.ownerTypeValue, companyCode[0])
        }
        ...
        if (hotelIds.length > 0) {
          ...
          this.searchComplete && this.searchComplete.completeScope && this.searchComplete.completeScope(this.scopeValue, hotelIds[0])
        } else {
          this.selecttargetObject = companyCode[0].name
          this.searchComplete && this.searchComplete.completeScope && this.searchComplete.completeScope(this.scopeValue, companyCode[0])
        }
      }
    },
    // 选择集团酒店
    selectObjs (type) {
      ...
      if (this.searchComplete && this.searchComplete.getTabName) {
        this.modalData.getTabName = this.searchComplete.getTabName
      }
    },
    // 删除选择的集团酒店
    cancleObs (type) {
  ...
  created () {
    ...
    this.searchData.ownerTypeNam = this.searchComplete.ownerTypeSelect
    this.searchData.scopeNam = this.searchComplete.scopeSelect
    if (this.searchComplete.ownerTypeSelect == 1) {
      this.ownType = ''
    } else if (this.searchComplete.ownerIdentifierSelect == '') {
      this.ownType = ''
    } else {
      this.ownType = this.searchComplete.ownerTypeSelect
    }
    if (this.searchComplete.scopeSelect == 1) {
      this.targetObject = ''
    } else if (this.searchComplete.scopeIdentifierSelect == '') {
      this.targetObject = ''
    } else {
      this.targetObject = this.searchComplete.scopeSelect
    }
    this.ownerTypeValue = this.searchComplete.ownerTypeSelect
    this.scopeValue = this.searchComplete.scopeSelect
    this.selectOwnTypeObject = this.searchComplete.ownerIdentifierSelect.name
    this.selecttargetObject = this.searchComplete.scopeIdentifierSelect.name
  }
}
</script>
