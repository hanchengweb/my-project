...
import { changeFirstUrl, getAccess, getCompanyCode, getCompany, setCompanyCode} from '../../libs/util'
...
export default {
  ...
  computed: {
    ...mapState({
      ...
      getCompanyChoseTr: state => state.data.companychoseTr,
  ...
  watch: {
    ...
    getChangeTableStatus (val) {
      // console.log(8888888)
      if (this.table.nameTitle && val) { // 用于搜索集团酒店后的重置搜索
        this.name = this.table.nameTitle
      }
      let _this = this
  ...
  methods: {
    ...
    getScrollHeight () {
      ...
      if (!this.table.hideButtonSearch) this.$refs.tableSelf.style.cssText = `height:calc(100% - 80px)`
      ...
      return {
        ...
        on: {
          click: (e) => {
            ...
            // if (this.table.type === 'sub' && !record.address) {
            //   this.handleCompanyChoseTr(record)
            // } else {
            //   this.handleCompanyChoseTr({})
            // }
          ...
          dblclick: (e) => {
            ...
            if (this.table.banDblClick && this.name != 'adMaterial') return false
            ...
            if (this.name == 'adMaterial') {
              if (this.getChoseTr.status == 1 || this.getChoseTr.auditStatus == 1) {
                this.$emit('show-modal', 'check')
              } else {
    ...
    getData (type) {
      ...
      this.$route.name == 'managerPlan' || this.$route.name == 'managerAdMaterial') {
        if (this.name == 'companylist2') {
          this.name = 'companylist'
          if (this.table.data.companyCode) {
            return false
          }
        } else if (this.table.name == 'companylist' && !this.npCompanylist) {
          this.name = 'companylist'
        } else if (this.table.name == 'hotelslist') {
          this.name = 'hotelslist'
        } else if (this.table.name == 'companylist' && this.npCompanylist) {
          return false
        }

        // if (this.table.name == 'companylist') {
        //   this.name = 'companylist'
        // }
      ...
      if (this.$route.name == 'programUser' && type == 'extra') {
        ...
        this.$route.name != 'groupSearch' && this.$route.name != 'hotelSearch' && this.$route.name != 'groupPlan' && this.$route.name != 'hotelPlan') {
      ...
      if (this.name === 'hotelslist') {
        ...
        // if (isEmptyObject(this.getCompanyChoseTr)) {
        //   data.companyCode = '123'
        // }
      ...
      getTable(url, data, typeAxios).then(res => {
        ...
        if (this.name == 'companylist' && res.data.content.length == 0 && this.$route.name != 'groupSearch') {
          setCompanyCode('123')
          setTimeout(function () {
